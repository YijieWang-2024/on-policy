param(
    ...
    [int]$MaxParallel = 3,
    [string]$ExperimentPrefix = "diag1500_crossreadout"
)
...
$EvalDir = Join-Path $RepoRoot "eval_outputs\cross_attention_readout_1500k"
...
$Jobs = @(
    [pscustomobject]@{
        Name = "Flat-HAP + Set-UAV pooled actor"
        Experiment = "${ExperimentPrefix}_flathap_setuav_pooled_seed${Seed}"
        Args = @(
    ...
    [pscustomobject]@{
        Name = "Flat-HAP + Set-UAV cross-attention actor"
        Experiment = "${ExperimentPrefix}_flathap_setuav_cross_seed${Seed}"
        Args = @(
            "--mec_policy_arch", "flat_hap_set_uav",
            "--mec_set_encoder_type", "mean_pool",
...
$Manifest = [ordered]@{
    ...
    purpose = "compare Set-UAV pooled, relational, and cross-attention readouts with Flat-HAP actor and Flat critic"
