...
$EvalDir = Join-Path $RepoRoot "eval_outputs\slot_eqdec_next_1500k"
...
$Jobs = @(
    [pscustomobject]@{
        Name = "Mean-pool slot EqDec, Flat critic, robustness seed 2"
        Experiment = "${ExperimentPrefix}_set_slot_flatcrit_seed2"
        Seed = 2
        Priority = 1
        Args = @(
            "--mec_policy_arch", "set",
    ...
    [pscustomobject]@{
        Name = "Mean-pool slot EqDec, Flat critic, robustness seed 3"
        Experiment = "${ExperimentPrefix}_set_slot_flatcrit_seed3"
        Seed = 3
        Priority = 1
        Args = @(
            "--mec_policy_arch", "set",
    ...
    [pscustomobject]@{
        Name = "Latent-slots EqDec, Flat critic, seed 1"
        Experiment = "${ExperimentPrefix}_latent_slot_flatcrit_seed1"
        Seed = 1
        Priority = 2
        Args = @(
            "--mec_policy_arch", "set",
    ...
    [pscustomobject]@{
        Name = "Mean-pool slot EqDec, Set critic separate encoder, seed 1"
        Experiment = "${ExperimentPrefix}_set_slot_setcrit_separate_seed1"
        Seed = 1
        Priority = 3
        Args = @(
            "--mec_policy_arch", "set",
    ...
    [pscustomobject]@{
        Name = "Mean-pool slot EqDec, Set critic shared-grad encoder, seed 1"
        Experiment = "${ExperimentPrefix}_set_slot_setcrit_sharedgrad_seed1"
        Seed = 1
        Priority = 4
        Args = @(
            "--mec_policy_arch", "set",
            ...
            "--mec_set_actor_context", "slot_attention",
            "--mec_critic_arch", "set",
            "--mec_set_critic_encoder", "shared_grad"
        )
    }
)
...
$Manifest = [ordered]@{
    ...
    purpose = (
        "continue slot-level EqDec: robustness seeds, latent slots, " +
        "and Set critic ownership diagnostics"
    )
    episode_length = $EpisodeLength
    steps = $Steps
...
while ($true) {
    ...
    $Completed = @()
    $Pending = @()
    foreach ($Job in ($Jobs | Sort-Object Priority)) {
        ...
        if (($null -ne $BestDir) -and
            (Get-RunReachedTarget -Experiment $Job.Experiment)) {
            $Completed += $Job
            continue
        }
        $Pending += $Job
    ...
    foreach ($Item in $Active) {
        Write-Output (
            ...
            "cpu=$($Item.process.CPU) rss_mb=" +
            "$([Math]::Round($Item.process.WorkingSet64 / 1MB, 1))"
        )
    }
