param(
    ...
    [string]$ExperimentPrefix = "resetperm1500_actor"
)
...
$EvalDir = Join-Path $RepoRoot "eval_outputs\resetperm_actor_baselines_1500k"
...
foreach ($Seed in 1..3) {
    $Jobs += [pscustomobject]@{
        Name = "Flat actor + Flat critic, reset permutation seed $Seed"
        Experiment = "${ExperimentPrefix}_flat_flat_seed${Seed}"
        Seed = $Seed
        Priority = 2 * $Seed - 1
        Args = @(
    ...
    $Jobs += [pscustomobject]@{
        Name = "Latent Slot-EqDec actor + Flat critic, reset permutation seed $Seed"
        Experiment = "${ExperimentPrefix}_latent_slot_flatcrit_seed${Seed}"
        Seed = $Seed
        Priority = 2 * $Seed
        Args = @(
            "--mec_policy_arch", "set",
...
$Manifest = [ordered]@{
    ...
    reset_permutation = $true
    purpose = (
        "rebuild Flat baseline and latent Slot-EqDec actor baseline " +
        "after reset-level UAV row permutation"
    )
    episode_length = $EpisodeLength
    steps = $Steps
